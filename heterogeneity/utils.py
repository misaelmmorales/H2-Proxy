...
class H2_heterogeneity:
    def __init__(self):
        ...
        self.fluv_range   = [0, 2.7]
        self.gaus_range   = [0,3.2]
    ...
    def process_perm_poro(self, lognormnoise=[-5,.1]):
        ...
        temp_fluv_norm = MinMaxScaler((self.fluv_range[0],self.fluv_range[1])).fit_transform(temp_fluv).T.reshape(self.standard_dim)
        ...
        temp_gaus_norm = MinMaxScaler((self.gaus_range[0],self.gaus_range[1])).fit_transform(temp_gaus).T.reshape(self.standard_dim)
        ...
        self.df_fluv = np.concatenate([np.expand_dims(self.perm_fluv,-1), np.expand_dims(self.poro_fluv,-1)], -1)
        self.df_gaus = np.concatenate([np.expand_dims(self.perm_gaus,-1), np.expand_dims(self.poro_gaus,-1)], -1)
        if self.verbose:
            print('Fluvial dataset shape:', self.df_fluv.shape)
            print('Gaussian dataset shape:', self.df_gaus.shape)
        if self.save_data:
            np.save('hete_fluv.npy', self.df_fluv)
            np.save('hete_gaus.npy', self.df_gaus)
        if self.return_data:
            return self.df_fluv, self.df_gaus
        
    def plot_samples(self, figsize=(20,6), cmaps=['jet','jet']):
        fig, axs = plt.subplots(4, len(self.selector), figsize=figsize)
        df = np.concatenate([self.df_fluv, self.df_gaus], -1)
        im, k = {}, 0
        for i in range(4):
            for j in range(len(self.selector)):
                ...
                if i%2==0:
                    im[k] = axs[i,j].imshow(df[j,:,:,i], cmap=cmaps[0])
                    axs[i,0].set(ylabel='Log-Perm')
                else:
                    im[k] = axs[i,j].imshow(df[j,:,:,i], cmap=cmaps[1])
